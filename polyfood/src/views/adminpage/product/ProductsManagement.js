...
const ProductsManagement = ({token}) => {
  ...
  const handleDelete = async (id) => {
    await axios.delete(
      `http://localhost:8080/api/v1/auth/admin/deleteProductById/${id.productId}`,
      { headers }
    );
    loadProducts();
  };
  ...
  const handleEdit = (record) => {
    navigate(`/adminuser/${token}/editProduct/${record.productId}`);
  };
  ...
  const defaultColumns = [
    ...
    },
    {
      ...
      render: (stars) => (
        <div className="flex">
          <Rate className="flex" allowHalf defaultValue={stars} />
        </div>
      ),
      editable: true,
    },
  ...
  const handleAddButton = async (record) => {
    const response = await axios.post(
      "http://localhost:8080/api/v1/auth/admin/addProduct",
      record,
      { headers }
    );
  };
